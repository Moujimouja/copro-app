...
class IncidentResponse(BaseModel):
    ...
    service_instance_names: List[str] = []  # Noms des équipements impactés
    ...
    def from_incident(cls, incident: Incident):
        ...
        # Charger les équipements via la table de liaison (nouveau système)
        if hasattr(incident, 'service_instances') and incident.service_instances:
            service_instance_names = [si.name for si in incident.service_instances]
            # Prendre le statut du premier équipement pour equipment_status (rétrocompatibilité)
            if incident.service_instances:
                equipment_status = incident.service_instances[0].status
        # Sinon, utiliser service_instance (ancien système pour rétrocompatibilité)
        elif incident.service_instance_id and incident.service_instance:
...
async def get_status_page(db: Session = Depends(get_db)):
    ...
    incidents = db.query(Incident).options(
        ...
        joinedload(Incident.service_instances)
    ).filter(
